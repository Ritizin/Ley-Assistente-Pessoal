...
export async function createAuthDatabase(options: AuthDatabaseOptions = {}): Promise<AuthDatabaseAdapter> {
  ...
  const sqliteFilePath = options.sqliteFilePath ?? DEFAULT_SQLITE_PATH;
  ...
  return {
    ...
    async createOrUpdateUser(input) {
      const now = Date.now();
      const existing = prepare("SELECT * FROM auth_users WHERE email = ?").get(input.email) as AuthUserRecord | undefined;
    ...
    async findUserByEmail(email) {
      const row = prepare("SELECT * FROM auth_users WHERE email = ?").get(email) as AuthUserRecord | undefined;
      return row ?? null;
    },
    async findUserByGoogleId(googleId) {
      const row = prepare("SELECT * FROM auth_users WHERE google_id = ?").get(googleId) as AuthUserRecord | undefined;
      return row ?? null;
    },
